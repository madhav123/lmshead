...
public class SynchronousCommandProcessingService implements CommandProcessingService {
    ...
    private NewCommandSourceHandler findCommandHandler(final CommandWrapper wrapper) {
        ...
        if (wrapper.isAccountTransferResource()) {
            ...
            }else if (wrapper.isCreateWithReschedule()) {
                handler = this.applicationContext.getBean("loanApplicationSubmittalCommandHandlerWithReschedule", NewCommandSourceHandler.class);
            } else if (wrapper.isUpdate()) {
            ...
            } else if (wrapper.isUpdateJobParam()) {
                handler = this.applicationContext.getBean("updateJobParameterCommandHandler", NewCommandSourceHandler.class);
            } else {
                throw new UnsupportedCommandException(wrapper.commandName());
            }
        ...
        } else if (wrapper.isEquityRessource()) {
            if (wrapper.isCreate()) {
                handler = this.applicationContext.getBean("createEquityCommandHandler", NewCommandSourceHandler.class);
            } else if (wrapper.isUpdate()) {
                handler = this.applicationContext.getBean("updateAssetCommandHandler", NewCommandSourceHandler.class);
            } else if (wrapper.isDelete()) {
                handler = this.applicationContext.getBean("deleteAssetCommandHandler", NewCommandSourceHandler.class);
