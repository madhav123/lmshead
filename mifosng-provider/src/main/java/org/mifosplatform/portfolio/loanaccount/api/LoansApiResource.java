...
public class LoansApiResource {
    ...
    private final Set<String> LOAN_DATA_PARAMETERS = new HashSet<String>(Arrays.asList("id", "accountNo", "status", "externalId",
            ...
            "loanCounter", "loanProductCounter", "notes", "accountLinkingOptions", "linkedAccount","loanIdid"));
    ...
    public LoansApiResource(final PlatformSecurityContext context, final LoanReadPlatformService loanReadPlatformService,
            ...
            final InsuranceReadPlatformService insuranceReadPlatformService,final LoanWritePlatformService loanWritePlatformService,
            final EquityReadPlatformService equityReadPlatformService
            ) {
    ...
    @GET
    @Path("{loanId}/printleasedoc")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_OCTET_STREAM })
    public Response printlease(@PathParam("loanId") final Long loanId, @Context final UriInfo uriInfo) {
        this.context.authenticatedUser().validateHasReadPermission(this.resourceNameForPermissions);
        String printFileName= loanReadPlatformService.printLeaseDetails(loanId);
    ...
    public String retrieveLoan(@PathParam("loanId") final Long loanId,
        ...
        Collection<EquityData>  equityes=null;
        ...
        if (!associationParameters.isEmpty()) {
        	 ...
        	 if (associationParameters.contains("all")) {
                 associationParameters.addAll(Arrays.asList("repaymentSchedule", "transactions", "charges", "guarantors", "collateral",
                         "notes", "linkedAccount", "insurances","equityes"));
             }
            ...
            if (associationParameters.contains("equityes")) {
                mandatoryResponseParameters.add("equityes");
                equityes= this.equityReadPlatformService.retrieveEquityDataAll(loanId);
                if (CollectionUtils.isEmpty(equityes)) {
                	insurances = null;
                }
            }
      ...
      BigDecimal principaPlusInterest=loanReadPlatformService.retrivePrincipalForReSchedule(loanId);
        ...
        final LoanAccountData loanAccount = LoanAccountData.associationsAndTemplate(loanBasicDetails, repaymentSchedule, loanRepayments,
                ...
                loanCollateralOptions, calendarOptions, notes,insurances,equityes, accountLinkingOptions, linkedAccount );
...
//            @QueryParam("underHierarchy") final String hierarchy,
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String calculateLoanScheduleOrSubmitLoanApplicationWithReschedule(@QueryParam("command") final String commandParam,
            @Context final UriInfo uriInfo, final String apiRequestBodyAsJson) {
        final JsonElement parsedQuery2 = this.fromJsonHelper.parse(apiRequestBodyAsJson);
        if (is(commandParam, "calculateLoanSchedule")) {
            final JsonElement parsedQuery = this.fromJsonHelper.parse(apiRequestBodyAsJson);
            final JsonQuery query = JsonQuery.from(apiRequestBodyAsJson, parsedQuery, this.fromJsonHelper);
            final LoanScheduleModel loanSchedule = this.calculationPlatformService.calculateLoanSchedule(query);
        ...
        final CommandWrapper commandRequest = new CommandWrapperBuilder().createLoanApplicationWithReschedule().withJson(apiRequestBodyAsJson).build();

        final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
        Long parentloanId=this.fromJsonHelper.extractLongNamed("loanIdid", parsedQuery2);
        if(parentloanId!=null)
        	loanWritePlatformService.closeAsRescheduledThroughCurrentdate(parentloanId);
